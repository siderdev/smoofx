...
(function ($) {
    ...
    $.tabs = function (el, options) {
        ...
        var setAriaAttributes = function () {
            $(plugin.settings.selectorList, $element).attr('role', 'tablist').find(plugin.settings.selectorListItems).attr('role', 'presentation').find(plugin.settings.selectorLinks).attr({
                'role': 'tab',
                'tabindex': '-1'
        ...
        plugin.switchClasses = function ($el) {
            ...
            // Current element
            $el.addClass(plugin.settings.classActive).attr({
                'aria-selected': 'true',
                'tabindex': '0'
            }).focus();
        };
        ...
        plugin.switchContent = function (targetId) {
            var $target;
            // Other elements
            $(plugin.settings.selectorContent, $element).attr({
                'aria-hidden': 'true'
            });
            // Target element
            if (targetId) {
